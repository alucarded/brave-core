...
class URLFetcher : public BATLedgerContext::Object {
 public:
  ...
  using FetchFuture = Future<mojom::UrlResponsePtr>;
  FetchFuture Fetch(mojom::UrlRequestPtr request);
  FetchFuture Fetch(mojom::UrlRequestPtr request, FetchOptions options);

 protected:
  virtual FetchFuture FetchImpl(mojom::UrlRequestPtr request,
