...
URLFetcher::FetchFuture URLFetcher::Fetch(mojom::UrlRequestPtr request) {
...
URLFetcher::FetchFuture URLFetcher::FetchImpl(mojom::UrlRequestPtr request,
                                              FetchOptions options) {
  DCHECK(request);
  LogURLRequest(*request, options);

  return FetchFuture::Create([&](auto resolver) {
    context().GetLedgerClient()->LoadURL(
        std::move(request),
        [weak_self = weak_factory_.GetWeakPtr(), resolver,
         options](const mojom::UrlResponse& response) mutable {
          if (weak_self)
            weak_self->LogURLResponse(response, options);
