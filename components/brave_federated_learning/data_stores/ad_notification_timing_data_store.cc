...
void BindSampleLogToStatement(
  ...
  s->BindInt64(3, notification_timing_log.creation_date.ToInternalValue());
}
...
bool AdNotificationTimingDataStore::AddLog(
  ...
  sql::Statement s(db_.GetUniqueStatement(
      base::StringPrintf(
          "INSERT INTO %s (time, locale, number_of_tabs, creation_date) "
          "VALUES (?,?,?,?)",
          task_name_.c_str())
          .c_str()));
...
void AdNotificationTimingDataStore::LoadLogs(
  ...
  while (s.Step()) {
    AdNotificationTimingTaskLog ntl(
        ...
        base::Time::FromInternalValue(s.ColumnInt64(4)));  // creation_date
