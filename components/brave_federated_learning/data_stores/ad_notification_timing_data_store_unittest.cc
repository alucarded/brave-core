...
class AdNotificationTimingDataStoreTest : public testing::Test {
 public:
  ...
  AdNotificationTimingDataStore* ad_notification_ds_;
};

void AdNotificationTimingDataStoreTest::SetUp() {
  ASSERT_TRUE(temp_dir_.CreateUniqueTempDir());
  base::FilePath db_path(temp_dir_.GetPath().Append("ad_notification_test"));
  ad_notification_ds_ = new AdNotificationTimingDataStore(db_path);
  ASSERT_TRUE(ad_notification_ds_->Init(
      "0.0.1", "ad_notification_timing_federated_task", 50, 30));
  ClearDB();
}

void AdNotificationTimingDataStoreTest::TearDown() {
  ad_notification_ds_ = nullptr;
}

void AdNotificationTimingDataStoreTest::ClearDB() {
  EXPECT_TRUE(ad_notification_ds_->DeleteAllLogs());
}

size_t AdNotificationTimingDataStoreTest::CountRecords() const {
  sql::Statement s(ad_notification_ds_->db_.GetUniqueStatement(
...
AdNotificationTimingDataStoreTest::AdNotificationTimingTaskLogFromTestInfo(
  ...
  return AdNotificationTimingTaskLog(
      ...
      info.number_of_tabs, base::Time::FromInternalValue(info.days_from_now));
}

void AdNotificationTimingDataStoreTest::AddAll() {
  ClearDB();
  for (size_t i = 0; i < base::size(ad_notification_task_log_test_db); ++i)
    ad_notification_ds_->AddLog(AdNotificationTimingTaskLogFromTestInfo(
...
TEST_F(AdNotificationTimingDataStoreTest, CheckSchemaColumnExistence) {
  ASSERT_TRUE(ad_notification_ds_->db_.DoesColumnExist(
...
TEST_F(AdNotificationTimingDataStoreTest, DeleteAllLogs) {
  AddAll();
  EXPECT_EQ(4U, CountRecords());
  AdNotificationTimingDataStore::IdToAdNotificationTimingTaskLogMap
      ad_notification_timing_logs;
  ad_notification_ds_->LoadLogs(&ad_notification_timing_logs);
  EXPECT_EQ(ad_notification_timing_logs.size(), CountRecords());
  EXPECT_TRUE(ad_notification_ds_->DeleteAllLogs());
  EXPECT_EQ(0U, CountRecords());
  ad_notification_ds_->LoadLogs(&ad_notification_timing_logs);
  EXPECT_EQ(0U, CountRecords());
